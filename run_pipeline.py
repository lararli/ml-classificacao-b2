...
def validate_configs():
    ...
    print(f"  {ok('[ok]')} data: {data.source}, {len(data.numerical)} num, {len(data.categorical)} cat")
    print(f"  {ok('[ok]')} pipeline: random_state={pipeline.random_state}, test_size={pipeline.test_size}")
    print(f"  {ok('[ok]')} quality: {len(quality.columns)} column checks")
    print(f"  {ok('[ok]')} experiments: {len(experiments.models)} models ({MODE})")
...
def check_quality(df, quality, run_id):
    ...
    for check in report:
        ...
        if check["passed"]:
            print(f"  {ok('[ok]')} {check['type']} ({col}): {check['detail']}")
        else:
            print(f"  {fail('[FAILED]')} {check['type']} ({col}): {check['detail']}")
    return report
...
def select_model(results, X_test, y_test, execution_id):
    ...
    print(f"\n{ok('selected:')} {bold(best['name'])} (f1={best['metrics']['f1_score']:.4f})")
    return best
...
def detect_drift(df, data, pipeline):
    ...
    for r in drift:
        if r["drift"]:
            print(f"  {warn('[DRIFT]')} {r['feature']}: var={r['variation_pct']}%")
        else:
            print(f"  {ok('[ok]')} {r['feature']}: var={r['variation_pct']}%")

    cats = check_new_categories(df_hist, df_prod, data.categorical)
    for r in cats:
        if r["has_new"]:
            print(f"  {warn('[ALERT]')} {r['feature']}: {r['new_categories']}")
...
def main():
    ...
    print(f"  mode={MODE}  id={execution_id}  models={len(results)}  best={bold(best['name'])} (f1={best['metrics']['f1_score']:.4f})")
