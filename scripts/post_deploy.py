...
import mlflow
from src.colors import ok, fail, warn, info, bold
...
def analyze():
    ...
    if runs.empty:
        print(fail("no production runs found.") + " Run 'make prod' first.")
        return

    if len(runs) < 2:
        current = runs.iloc[0]
        print(f"\n{bold('POST-DEPLOY ANALYSIS')}")
        ...
        print(f"  {info('no previous run to compare. This is the first deployment.')}")
        return
    ...
    if len(runs) < 2:
        print(f"  {info('only one execution_id found. Need at least 2 monthly runs to compare.')}")
        return
    ...
    for m in metrics:
        ...
        if m == "train_time_seconds":
            ...
            if m == "f1_score" and diff < -0.02:
                change = fail(f"{sign}{diff:.4f} !")
                degraded = True
            elif diff > 0:
                change = ok(f"{sign}{diff:.4f}")
            elif diff < -0.01:
                change = warn(f"{sign}{diff:.4f}")

        print(f"  {m:<20} {prev_val:>18.4f} {cur_val:>18.4f} {change:>12}")

    if degraded:
        print(f"\n  {fail('STATUS: DEGRADED')}. f1 dropped more than 0.02. Investigate drift.")
        print(f"  {warn('ACTION:')} run 'make test' to re-evaluate all models.")
    else:
        print(f"\n  {ok('STATUS: STABLE')}. Model performing within expected range.")
